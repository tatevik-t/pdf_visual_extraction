        ...
        for element in elements:
            if element.get("type") == "table":
                table_data = {
                    ...
                    "structured_data": element.get("content", {}).get(
                        "structured_data", ""
                    ),
...
def inject_tables_into_text(
    text_data: Dict[str, Any], tables_by_page: Dict[int, List[Dict[str, Any]]]
) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject table data into text extraction JSON"
    )
    parser.add_argument(
        "--text_file", required=True, help="Path to text extraction JSON file"
    )
    parser.add_argument(
        "--visual_file", required=True, help="Path to visual detection JSON file"
    )
    parser.add_argument(
        "--output_file", required=True, help="Path to output enhanced JSON file"
    )
