...
def convert_tables_to_csv(data: Dict[str, Any], output_dir: str, pdf_name: str) -> Dict[str, Any]:
    """Convert all extracted tables to CSV format"""
    
    # Initialize OpenAI client with API key from environment
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key:
        raise ValueError("OPENAI_API_KEY environment variable is required for CSV conversion")
...
def main():
    parser = argparse.ArgumentParser(description='Convert extracted tables to CSV format')
    parser.add_argument('--data_file', required=True, help='Path to JSON file with table data')
    ...
    try:
        ...
        results = convert_tables_to_csv(data, args.output_dir, args.pdf_name)
        
        print(f"✅ CSV conversion completed!")
        print(f"   Summary saved to: {os.path.join(args.output_dir, 'csv_exports', f'{args.pdf_name}_csv_conversion_summary.json')}")
