...
def convert_pdf_to_images(
    ...
    try:
        ...
        images = pdf2image.convert_from_path(
            ...
            thread_count=4,
            first_page=1,
            last_page=max_pages if max_pages is not None else 999999,
        )

        # Generate image file paths
        # pdf_name = Path(pdf_path).stem  # Not used
        image_paths = []
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        help="Output directory for images (default: pdf_name_images)",
    )
    parser.add_argument(
    ...
    if image_paths:
        print(
            f"Conversion completed! Generated {len(image_paths)} images in: {output_dir}"
        )
    else:
